...
const ProgressBar = memo(({ start, end, user }: ProgressProps) => {
	...
	return (
		<>
			<div className="h-1 w-full overflow-hidden rounded-sm bg-gray-400">
				<div
					className="h-full rounded-sm bg-[#1DB954] transition-all duration-1000 ease-linear will-change-[width] dark:bg-[#00ff00]"
					style={{ width: `${progress}%` }}
				></div>
			</div>
			<div className="mx-1">
				{formatTime(dayjs(new Date().getTime() - user.spotify.timestamps.start).valueOf() / 1000)}
			</div>
		</>
	);
});
	...
	return (
		<>
			<motion.div
				...
				className="move-position fixed bottom-12 left-6 hidden h-[7rem] flex-col items-start justify-start font-sans transition-opacity laptop:flex"
			>
				...
				<div className="flex h-[6rem] w-full flex-row items-center justify-start rounded-lg bg-white/60 p-2 backdrop-blur-lg dark:bg-[#5f5555ad]">
					<div className="mr-1 w-fit scale-95 transition duration-500 hover:scale-100 hover:ease-out">
						<Image
							src={user.spotify.album_art_url}
							className="pointer-events-none h-[4.5rem] w-[4.5rem] max-w-[4.5rem] animate-spin-slow rounded-full bg-cover" //animate-spin-slow
			...
			<motion.div
				...
				className="fixed bottom-0 z-[1] flex w-full flex-col items-start justify-start bg-white/60 px-6 py-2 backdrop-blur-lg dark:bg-[#5f5555ad] laptop:hidden"
			>
